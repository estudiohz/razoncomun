...
// (manifiesto.mjs, corpusStorage.mjs...), trocea, genera embeddings y hace un
...
export async function ingestDocuments(documents, { dryRun = false, log = console.log } = {}) {
  ...
  for (const doc of documents) {
    ...
    if (!dryRun) {
      ...
      // Gancho opcional para que un connector reaccione a una indexación con
      // éxito de ESTE documento concreto -- p.ej. brainEntries.mjs lo usa para
      // marcar brain_entries.indexed_at = now() SOLO después de que los
      // chunks ya están insertados de verdad (nunca antes, para no marcar
      // "indexado" algo que falló a medias). Se ignora en dry-run porque no
      // se escribió nada. Los connectores existentes (manifiesto, docs) no lo
      // usan -- doc.onIndexed es undefined para ellos y este bloque no hace nada.
