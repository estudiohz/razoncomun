...
import { fetchCorpusDocDocuments } from "./connectors/corpusDocs.mjs";
...
async function main() {
  ...
  if (args.sources.includes("docs")) {
    console.log(`· Leyendo corpus de documentos (${config.corpusPublicDir}, ${config.corpusInternalDir})...`);
    const docs = await fetchCorpusDocDocuments();
