...
//   node src/index.mjs --sources=conocimiento              # solo brain_entries pendientes (indexed_at IS NULL)
//   node src/index.mjs --sources=conocimiento --all         # reindexado COMPLETO de brain_entries
//
...
import { fetchBrainEntriesDocuments } from "./connectors/brainEntries.mjs";
import { ingestDocuments } from "./ingest.mjs";

function parseArgs(argv) {
  const args = { sources: ["manifiesto", "docs"], dryRun: false, reindexAll: false };
  for (const arg of argv) {
    if (arg === "--dry-run") args.dryRun = true;
    else if (arg === "--all") args.reindexAll = true;
...
async function main() {
  ...
  if (args.sources.includes("conocimiento")) {
    console.log(
      `· Leyendo brain_entries (wiki de conocimiento)${
        args.reindexAll ? " [--all: reindexado COMPLETO]" : " [solo pendientes: indexed_at IS NULL]"
      }...`
    );
    const { documents: entryDocs, skipped, totalEntries } = await fetchBrainEntriesDocuments({
      all: args.reindexAll,
    });
    console.log(
      `  ${totalEntries} entrada(s) encontradas · ${entryDocs.length} a indexar · ${skipped.length} saltada(s)`
    );
    if (skipped.length > 0) {
      for (const s of skipped) console.log(`    ⚠️  ${s.id} "${s.title}" -- ${s.reason}`);
    }
    documents.push(...entryDocs);
  }
