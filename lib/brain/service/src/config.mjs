...
export const config = {
  ...
  // --- Desambiguación por tema ---
  // Cuando una pregunta AMPLIA (p.ej. solo "autónomos") no cruza el umbral de
  // respuesta pero roza VARIAS entradas de conocimiento distintas y comparables,
  // en vez de "no lo sé" ofrecemos elegir tema (botones). `floor` = suelo por
  // debajo del cual es ruido y no se ofrece nada; `margin` = cuánto puede bajar
  // la 2ª entrada respecto a la 1ª para considerarlas "comparables" (mismo grado
  // de relevancia = pregunta ambigua). Solo actúa en el tramo que HOY sería
  // "fuera de corpus": nunca cambia una respuesta que ya cruza el umbral.
  disambiguationFloor: parseFloat(process.env.DISAMBIGUATION_FLOOR || "0.40"),
  disambiguationMargin: parseFloat(process.env.DISAMBIGUATION_MARGIN || "0.15"),
