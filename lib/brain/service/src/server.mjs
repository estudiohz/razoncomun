...
async function handlePublicChat(req, res, body) {
  ...
  if (outOfCorpus) {
    ...
    const { suggestions, debug: _dbg } = await disambiguationFor(message);
    if (suggestions) {
      await logChatTurn({
        channel: "web",
        sessionId,
    ...
    // _debugDisambig: temporal, para calibrar umbrales con datos reales. Quitar tras calibrar.
    return sendJson(res, 200, { answer: OUT_OF_CORPUS_MESSAGE, sources: [], outOfCorpus: true, flaggedInjection: false, _debugDisambig: _dbg });
  }
