...
import { classifyAndStoreContribution } from "./contributions.mjs";
...
async function handlePublicChat(req, res, body) {
  ...
  if (outOfCorpus) {
    // Antes de rendirse con "no lo sé": si la pregunta amplia roza varias
    // entradas de conocimiento comparables (p.ej. "autónomos" -> cuota +
    // vacaciones), ofrecemos elegir tema en vez del mensaje de fuera de corpus.
    const { suggestions } = await disambiguationFor(message);
    if (suggestions) {
      await logChatTurn({
        channel: "web",
        sessionId,
        ipHash: ipHash(ip),
        userText: message,
        answerText: DISAMBIGUATION_MESSAGE,
        sources: [],
        outOfCorpus: false,
        flaggedInjection: false,
      }).catch((err) => console.error("[audit] fallo registrando turno (desambiguación):", err.message));
      return sendJson(res, 200, {
        answer: DISAMBIGUATION_MESSAGE,
        sources: [],
        outOfCorpus: false,
        flaggedInjection: false,
        suggestions,
  ...
  const entryTitles = await entryTitlesForRows(rows);
  const sources = rows.map((r) => {
    const titulo = r.source === "conocimiento" && r.ref_id ? entryTitles.get(r.ref_id) : null;
    return {
      label: titulo || describeSource(r),
      url: titulo ? `/cerebro/${r.ref_id}` : null,
      source: r.source,
      similarity: Number(r.similarity),
    };
  });
  const charts = await chartsForRows(rows);
  const embeds = await embedsForRows(rows);
  const related = await relatedForRows(rows);
  let answer;
  try {
    ...
    if (err && err.rateLimited) {
      // Cuota del proveedor de IA agotada (p. ej. free tier de Gemini): aviso
      // amable con los segundos sugeridos, en vez del error genérico.
      const secs = err.retryAfterSeconds || 30;
      return sendJson(res, 429, {
        error: `La IA está recibiendo muchas peticiones ahora mismo. Reinténtalo en unos ${secs} segundos.`,
        rateLimited: true,
  ...
  // El front ya no necesita el pie "[1] etiqueta" en texto plano: `sources`
  // (arriba) trae label+url por fuente y el propio componente de chat pinta
  // una lista de citas clicable. `answerWithSources` se sigue guardando en
  // la auditoría (logChatTurn) para no perder trazabilidad histórica.
  return sendJson(res, 200, { answer: answer.trim(), sources, outOfCorpus: false, flaggedInjection: false, charts, embeds, related });
}

/**
 * Título real de cada entrada de la wiki (brain_entries, ref_id) citada como
 * fuente, para que el chat muestre "Autónomos: cuota cero" en vez del nombre
 * genérico de la tabla ("conocimiento"). Fallo de lectura => sin títulos,
 * `describeSource` hace de respaldo — nunca tumba la respuesta.
 */
async function entryTitlesForRows(rows) {
  const ids = [...new Set(rows.filter((r) => r.source === "conocimiento" && r.ref_id).map((r) => r.ref_id))];
  if (ids.length === 0) return new Map();

  const inList = ids.map((id) => escapeStringLiteral(id)).join(",");
  try {
    const titRows = await pgQuery(
      `select id, title from brain_entries where visibility = 'public' and id in (${inList});`
    );
    return new Map(titRows.map((t) => [t.id, t.title]));
  } catch (err) {
    console.error("[chat] fallo leyendo títulos de fuentes:", err.message);
    return new Map();
  }
}

/**
 * Entradas de conocimiento RELACIONADAS con la respuesta: las recuperadas junto
 * a la principal (misma temática — p. ej. un artículo dividido en dos, o dos
 * propuestas cercanas). Se ofrecen como opción clicable BAJO la respuesta
 * directa, para que el usuario salte al tema secundario. Excluye la principal
 * (la de mayor similitud, sobre la que se ha respondido) y limita a 2. Fallo de
 * lectura => sin relacionadas, nunca tumba la respuesta.
 */
async function relatedForRows(rows) {
  const bySim = new Map();
  for (const r of rows) {
    if (r.source === "conocimiento" && r.ref_id) {
      const s = Number(r.similarity) || 0;
      if (!bySim.has(r.ref_id) || s > bySim.get(r.ref_id)) bySim.set(r.ref_id, s);
    }
  }
  const entries = [...bySim.entries()].map(([id, sim]) => ({ id, sim })).sort((a, b) => b.sim - a.sim);
  if (entries.length < 2) return [];

  const rel = entries.slice(1).filter((e) => e.sim >= config.disambiguationFloor).slice(0, 2);
  if (rel.length === 0) return [];

  const inList = rel.map((e) => escapeStringLiteral(e.id)).join(",");
  let titRows;
  try {
    titRows = await pgQuery(
      `select id, title from brain_entries where visibility = 'public' and id in (${inList});`
    );
  } catch (err) {
    console.error("[chat] fallo leyendo entradas relacionadas:", err.message);
    return [];
  }

  const titleById = new Map(titRows.map((t) => [t.id, t.title]));
  const out = [];
  for (const e of rel) {
    const title = (titleById.get(e.id) || "").trim();
    if (!title) continue;
    const idx = title.indexOf("?");
    const label = idx >= 0 ? title.slice(0, idx + 1) : title.length > 90 ? title.slice(0, 88) + "…" : title;
    out.push({ label, query: title });
  }
  return out;
}

/**
 * Simuladores HTML adjuntos a las entradas de la wiki recuperadas (0027). Solo
 * chunks de source='conocimiento' (ref_id -> brain_entries) con embed_html no
 * nulo. Devuelve SOLO { entryId, title } -- NUNCA el HTML (que el front carga
 * bajo demanda desde /api/cerebro/embed/[id]). Preserva el orden de similitud.
 * Fallo de lectura => sin simuladores, nunca tumba la respuesta del chat.
 */
async function embedsForRows(rows) {
  const orderedIds = [];
  for (const r of rows) {
  ...
  try {
    entryRows = await pgQuery(
      `select id, embed_title from brain_entries where visibility = 'public' and embed_html is not null and id in (${inList});`
    );
  } catch (err) {
    console.error("[chat] fallo leyendo embeds de brain_entries:", err.message);
    return [];
  }

  const titleById = new Map(entryRows.map((e) => [e.id, e.embed_title || "Simulador"]));
  const out = [];
  for (const id of orderedIds) {
    if (titleById.has(id)) out.push({ entryId: id, title: titleById.get(id) });
  }
...
const DISAMBIGUATION_MESSAGE =
  "Sobre eso tengo varios temas. ¿De cuál quieres que te hable? Pincha una opción:";

/**
 * Desambiguación por tema. Solo se llama cuando la respuesta caería "fuera de
 * corpus". Hace su PROPIA recuperación con un límite más alto (para no depender
 * de los 5 chunks del path de respuesta, que pueden no incluir el 2º tema), y
 * si la pregunta amplia roza VARIAS entradas de conocimiento comparables (por
 * encima de `disambiguationFloor` y con la 2ª dentro de `disambiguationMargin`
 * de la 1ª), devuelve opciones {label, query}. `debug` lista TODAS las entradas
 * de conocimiento halladas con su similitud, para calibrar umbrales (temporal).
 */
async function disambiguationFor(message) {
  let rows;
  try {
    rows = await retrieve(message, { visibility: "public", limit: 15 });
  } catch (err) {
    console.error("[chat] fallo recuperando para desambiguación:", err.message);
    return { suggestions: null, debug: [] };
  }

  const bySim = new Map(); // ref_id -> mejor similitud
  for (const r of rows) {
    if (r.source === "conocimiento" && r.ref_id) {
      const s = Number(r.similarity) || 0;
      if (!bySim.has(r.ref_id) || s > bySim.get(r.ref_id)) bySim.set(r.ref_id, s);
  ...
  const todas = [...bySim.entries()].map(([id, sim]) => ({ id, sim })).sort((a, b) => b.sim - a.sim);

  let titleById = new Map();
  if (todas.length) {
    const inList = todas.map((e) => escapeStringLiteral(e.id)).join(",");
    try {
      const titRows = await pgQuery(
        `select id, title from brain_entries where visibility = 'public' and id in (${inList});`
      );
      titleById = new Map(titRows.map((t) => [t.id, t.title]));
    } catch (err) {
      console.error("[chat] fallo leyendo títulos para desambiguación:", err.message);
    }
  }

  const debug = todas.map((e) => ({
    title: titleById.get(e.id) || "(sin título)",
    sim: Number(e.sim.toFixed(3)),
  }));

  const eligibles = todas.filter((e) => e.sim >= config.disambiguationFloor);
  if (eligibles.length < 2 || eligibles[1].sim < eligibles[0].sim - config.disambiguationMargin) {
    return { suggestions: null, debug };
  }

  const suggestions = [];
  for (const e of eligibles.slice(0, 4)) {
    const title = (titleById.get(e.id) || "").trim();
    if (!title) continue;
    // Etiqueta legible: la parte-pregunta del título (hasta el primer "?") o el
    // título recortado. La `query` reenviada al pinchar es el título completo.
    const idx = title.indexOf("?");
    const label = idx >= 0 ? title.slice(0, idx + 1) : title.length > 90 ? title.slice(0, 88) + "…" : title;
    suggestions.push({ label, query: title });
  }

  return { suggestions: suggestions.length >= 2 ? suggestions : null, debug };
}
...
// ---------------------------------------------------------------------------
// /classify-contribution -- SERVIDOR-A-SERVIDOR (X-Ingest-Secret). Lo
// dispara la web tras insertar una contribución ciudadana (pieza C). Lee la
// fila por id, la triA con la IA (injectionGuard antes del LLM) y actualiza
// ai_triage + status. No es un canal de usuario final: su única puerta es el
// secreto, igual que /admin/ingest.
// ---------------------------------------------------------------------------
async function handleClassifyContribution(req, res, body) {
  const auth = requireIngestSecret(req);
  if (!auth.ok) return sendJson(res, auth.status, { ok: false, error: auth.error });

  const contributionId = typeof body.contributionId === "string" ? body.contributionId.trim() : "";
  if (!contributionId) return sendJson(res, 400, { ok: false, error: "Falta 'contributionId'." });

  try {
    const row = await classifyAndStoreContribution(contributionId);
    return sendJson(res, 200, { ok: true, contribution: row });
  } catch (err) {
    if (err instanceof AnthropicNotConfiguredError) {
      return sendJson(res, 503, { ok: false, error: "El clasificador está pendiente de configurar (proveedor IA)." });
    }
    console.error("[classify-contribution] error:", err.message);
    return sendJson(res, 502, { ok: false, error: err.message });
  }
}
...
const routes = {
  ...
  "POST /classify-contribution": handleClassifyContribution,
