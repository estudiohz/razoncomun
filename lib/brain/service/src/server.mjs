...
async function handlePublicChat(req, res, body) {
  ...
  if (outOfCorpus) {
    ...
    const suggestions = await disambiguationForRows(rows);
    if (suggestions) {
      await logChatTurn({
        channel: "web",
        sessionId,
 ...
 * corpus": si la pregunta amplia roza VARIAS entradas de conocimiento distintas
 * y de relevancia comparable (por encima de `disambiguationFloor` y con la 2ª
 * dentro de `disambiguationMargin` de la 1ª), devuelve una lista de opciones
 * {label, query} para que el usuario elija, en vez del "no lo sé". Devuelve null
 * si no aplica (una sola entrada, o relevancias dispares). Fallo de lectura de
 * títulos => null (se cae con gracia al mensaje de fuera de corpus).
 */
async function disambiguationForRows(rows) {
  ...
  const entries = [...bySim.entries()]
    .map(([id, sim]) => ({ id, sim }))
    .filter((e) => e.sim >= config.disambiguationFloor)
    .sort((a, b) => b.sim - a.sim);

  if (entries.length < 2) return null;
  if (entries[1].sim < entries[0].sim - config.disambiguationMargin) return null;

  const candidatos = entries.slice(0, 4);
  const inList = candidatos.map((e) => escapeStringLiteral(e.id)).join(",");
  let titRows;
  try {
  ...
  for (const e of candidatos) {
    ...
    if (!title) continue;
    // Etiqueta legible: la parte-pregunta del título (hasta el primer "?") o,
    // si no es una pregunta, el título recortado. La `query` que se reenvía al
    // pinchar es el título completo (recupera esa entrada con alta similitud).
