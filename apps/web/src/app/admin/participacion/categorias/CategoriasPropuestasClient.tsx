...
import {
  crearCategoriaPropuesta,
  editarCategoriaPropuesta,
  eliminarCategoriaPropuesta,
  reordenarCategoriaPropuesta,
  type ResultadoAccion,
} from './actions';
import { cn } from '@/lib/cn';

export interface FilaCategoriaPropuesta {
  id: string;
  nombre: string;
  color: string;
  propuestas: number;
...
function FilaEditable({ fila }: { fila: FilaCategoriaPropuesta }) {
  ...
  function guardar() {
    setError(null);
    iniciar(async () => {
      const r = await editarCategoriaPropuesta(fila.id, nombre, color);
      if (!r.ok) {
  ...
  if (!editando) {
    return (
      <div className="flex min-w-0 flex-1 items-center gap-2">
        <span className="h-4 w-4 shrink-0 rounded-full border border-linea" style={{ backgroundColor: fila.color }} />
        <span className="truncate font-bold text-titular">{fila.nombre}</span>
        <button
          type="button"
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-2 min-[640px]:flex-row min-[640px]:items-center">
      <Input value={nombre} onChange={(e) => setNombre(e.target.value)} className="py-1.5 text-[14px]" autoFocus />
      <div className="flex shrink-0 items-center gap-2">
        <input
          type="color"
          value={/^#[0-9a-f]{6}$/i.test(color) ? color : '#8B30D9'}
          onChange={(e) => setColor(e.target.value)}
          className="h-9 w-9 shrink-0 cursor-pointer rounded border border-linea bg-white p-0.5"
          aria-label="Color de la categoría"
        />
        <Input
          value={color}
          onChange={(e) => setColor(e.target.value)}
          className="w-[100px] py-1.5 text-[13px]"
          placeholder="#8B30D9"
        />
      </div>
      <div className="flex shrink-0 items-center gap-2">
...
export function CategoriasPropuestasClient({ filas }: { filas: FilaCategoriaPropuesta[] }) {
  const router = useRouter();
  const [montado, setMontado] = useState(false);
  const [borrando, setBorrando] = useState<FilaCategoriaPropuesta | null>(null);
  ...
  function mover(id: string, direccion: 'subir' | 'bajar') {
    iniciarAccion(async () => {
      await reordenarCategoriaPropuesta(id, direccion);
      router.refresh();
    });
  }
  ...
  function confirmarBorrado() {
    ...
    iniciarAccion(async () => {
      const r = await eliminarCategoriaPropuesta(borrando.id);
      if (!r.ok) {
  ...
  return (
    <div>
          ...
          >
            ...
            <div className="flex shrink-0 items-center gap-2">
              <span className="rounded-full bg-white px-2.5 py-0.5 text-[12px] font-semibold text-cuerpo ring-1 ring-linea">
                {f.propuestas} propuesta{f.propuestas === 1 ? '' : 's'}
              </span>
      ...
      <div className="rounded-tarjeta border border-linea bg-white p-5">
        <h2 className="mb-3 text-[13px] font-bold uppercase tracking-[.08em] text-gris">Nueva categoría</h2>
        <form action={accionAlta} className="flex flex-col gap-3 min-[640px]:flex-row">
          <Input name="nombre" placeholder="Nombre de la categoría" required className="min-[640px]:flex-1" />
          <input
            type="color"
            name="color"
            defaultValue="#8B30D9"
            className="h-11 w-14 shrink-0 cursor-pointer rounded border border-linea bg-white p-0.5"
            aria-label="Color de la categoría"
          />
          <button
            type="submit"
      ...
      {borrando &&
        ...
        createPortal(
          ...
          >
            ...
            >
              <h2 className="text-[19px] font-bold text-titular">¿Eliminar «{borrando.nombre}»?</h2>
              <p className="mt-2 text-[14px] text-cuerpo">
                {borrando.propuestas > 0
                  ? `Esta categoría tiene ${borrando.propuestas} propuesta${borrando.propuestas === 1 ? '' : 's'}. Cámbialas de categoría antes de eliminarla.`
              ...
              <div className="mt-5 flex justify-end gap-3">
                ...
                <button
                  ...
                  disabled={pendienteAccion || borrando.propuestas > 0}
